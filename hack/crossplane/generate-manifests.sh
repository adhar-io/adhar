...
helm template crossplane --namespace $CROSSPLANE_NAMESPACE crossplane-preview/crossplane --version "$CROSSPLANE_VERSION" -f "$HACK_DIR/values.yaml" --include-crds > "$INSTALL_YAML"
helm template crossplane --namespace $CROSSPLANE_NAMESPACE crossplane-preview/crossplane --version "$CROSSPLANE_VERSION" -f "$HACK_DIR/values-ha.yaml" --include-crds > "$INSTALL_HA_YAML"
