...
5. Create test users `user1@noreply.com` (→`/platform-admin`) and `user2@noreply.com` (→`/platform-developer`); set their passwords from `USER_PASSWORD` via `reset-password`. The usernames are the emails because the realm enables `registrationEmailAsUsername`.
