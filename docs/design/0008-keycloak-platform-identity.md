...
- **Test users** `user1@noreply.com` → `/platform-admin`, `user2@noreply.com` → `/platform-developer` (passwords from the ESO-generated `USER_PASSWORD`). The usernames are the email addresses: the realm sets `registrationEmailAsUsername: true`, so Keycloak replaces the submitted username with the email.
...
- **Manual** — `adhar auth login user1@noreply.com --insecure && adhar auth whoami` should show `platform-admin`; `kubectl --token "$(adhar auth token)" auth can-i '*' '*'` should be `yes` (via `oidc:platform-admin → cluster-admin`).
