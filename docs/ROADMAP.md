...
- 🔜 **Control-plane / data-plane separation** ([ADR-0023](adr/0023-control-dataplane-separation.md), [plan](design/0023-control-dataplane-separation.md)): make the two cluster roles first-class and enforced — the control plane runs only fleet/platform services; **all application workloads run on data planes**. A new `DataPlane` API (`platform.adhar.io/v1alpha1`) owns each managed data plane's lifecycle (provision → register with ArgoCD → thin-agent profile → Cilium Mesh + SPIFFE → observability hub → aggregate status), giving a real "one control plane, many data planes" relationship. Package catalog split by an `adhar.io/plane` label (control-plane appset vs workload appsets); app→data-plane placement declared in the `environments` repo and enforced by Sveltos; a control-plane Kyverno policy keeps application workloads off `adhar-system`. Parity preserved at T1 via a **vcluster local data plane** ([ADR-0016](adr/0016-vcluster-local-first-development.md)), so apps run on a data plane even on a laptop; staged, reversible migration for existing single-cluster installs (`adhar migrate split-planes`)
...
- 🔜 **Agentic platform (Adhar AI)** ([ADR-0024](adr/0024-agentic-ai-platform.md), [plan](design/0024-agentic-ai-platform.md)): a fully agentic Adhar — configure one LLM key and the platform can investigate, explain, scaffold, provision, and remediate, **through** its existing control surfaces, never around them. Built as a separate Python repo (`adhar-io/adhar-ai`) shipped as a control-plane package:
  - 🔜 **MCP-native tools**: per-domain MCP servers (cluster, gitops, provision, observability, security, cost, catalog) with read tools RBAC-scoped and **write tools that only open Gitea PRs** — no direct `kubectl apply`/cloud-mutation path; the same servers are exposed outward (Gateway + OIDC) so external agents (Claude Code, IDEs, ChatOps) drive Adhar through identical governed tools
  - 🔜 **GitOps-safe agent runtime**: assistant mode (Console chat + `adhar ai` CLI) and event-driven operators (alert-triage, drift-explain, cost-advisor, upgrade-preflight) with staged autonomy (read-only → suggest → approve-to-apply → scoped) — every change lands as a reviewable, revertible Git PR
  - 🔜 **Provider-agnostic LLM gateway**: one secret (`adhar-ai-llm`, Vault→ESO) selects Claude (default, latest models) / OpenAI / Azure / OpenAI-compatible / Ollama (air-gapped); token/rate budgets central
  - 🔜 **Grounding & governance**: pgvector RAG over docs/ADRs/runbooks + live state; Keycloak identity, Kyverno policy on agent actions, full audit of every tool call to the observability hub; platform runs unaffected when uninstalled or unkeyed
