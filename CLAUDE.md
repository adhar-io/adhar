...
- **Single sign-on needs a SHARED cookie on the PARENT domain**: every oauth2-proxy reads the one `adhar-sso-cookie` Secret (`security/keycloak/manifests/sso-cookie.yaml`) and sets `--cookie-domain=.<host>` + `--whitelist-domain`. Each app used to mint its own cookie secret with no cookie domain, so the session was host-scoped AND undecryptable by any sibling — signing in to the console and opening another app forced a fresh auth round trip. Guarded by `TestEveryOAuth2ProxySharesTheSSOSessionCookie`. Apps with native OIDC (Argo CD, Gitea, Harbor) still render their own provider button; Grafana sets `auto_login` to skip its
