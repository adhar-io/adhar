...
func NewCluster(name, kubeVersion, kubeConfigPath, kindConfigPath, extraPortsMapping string, registryConfig []string, cfg v1alpha1.BuildCustomizationSpec) (*Cluster, error) {
	detectOpt, err := utils.DetectKindNodeProvider()
	if err != nil {
		return nil, err
	}

	// Create a no-op logr.Logger for kind
	noopLogger := logr.Discard()
	kindLogger := KindLoggerFromLogr(&noopLogger)
	provider := cluster.NewProvider(cluster.ProviderWithLogger(kindLogger), detectOpt)
