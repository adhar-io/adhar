...
* **port** — `http` (the Service's named port; the Go service's HTTP port)
* **path** — `/metrics`
...
The scaffolded `main.go` serves plain `net/http`. To keep the dashboard
populated after you replace the placeholder image, register the Prometheus
handler on the same mux and the same port:
...
Use the `promhttp` middleware (or `otelhttp` with the Prometheus exporter) so
that `http_requests_total` and `http_request_duration_seconds` keep the same
names and the `status` label the dashboard filters on.
