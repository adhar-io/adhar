...
1. **MCP-native tools** — seven per-domain MCP servers (`cluster`, `gitops`, `provision`,
   ...
   **Read** tools are RBAC-scoped (get/list/watch via the `adhar-ai` ServiceAccount). **Write**
   ...
   [`agentgateway`](../agentgateway/) ([ADR-0025](../../../../../docs/adr/0025-ai-gateway-agentgateway.md)),
...
3. **LLM access through the platform AI data plane (external to this package)** — completions go
   to [`agentgateway`](../agentgateway/) at `http://adhar-ai-gateway.adhar-system.svc.cluster.local:8080/v1`
   ...
   the in-cluster [`vllm`](../vllm/). One secret (`adhar-ai-llm`, Vault→ESO) still holds the
...
- **Identity + audit.** Keycloak gates access — once, in the AI data plane, for every LLM call and
  ...
  `agentgateway`'s CEL authorization reads; read tools need `platform-developer`, PR-opening
...
enable it together with `agentgateway`, which is what serves its LLM and MCP endpoints.
