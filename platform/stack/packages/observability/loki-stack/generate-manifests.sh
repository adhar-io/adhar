...
helm repo update
helm template --namespace adhar-system loki grafana/loki-stack --set fluent-bit.enabled=true,promtail.enabled=false -f values.yaml --version ${CHART_VERSION} --include-crds >>${INSTALL_YAML}