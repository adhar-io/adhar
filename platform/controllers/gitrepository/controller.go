...
func GetGitProvider(ctx context.Context, repo *v1alpha1.GitRepository, kubeClient client.Client, scheme *runtime.Scheme, tmplConfig v1alpha1.BuildCustomizationSpec) (gitProvider, error) {
	...
	case v1alpha1.GitProviderGitea:
		giteaClient, err := NewGiteaClient(repo.Spec.Provider.GitURL, gitea.SetHTTPClient(utils.GetHttpClient()))
		if err != nil {
			return nil, err
		}
...
// +kubebuilder:rbac:groups=platform.adhar.io,resources=gitrepositories,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=platform.adhar.io,resources=gitrepositories/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=platform.adhar.io,resources=gitrepositories/finalizers,verbs=update
...
func (r *GitRepositoryReconciler) reconcileGitRepo(ctx context.Context, repo *v1alpha1.GitRepository) (ctrl.Result, error) {
	...
	if r.Config.StaticPassword {
		creds.password = utils.StaticPassword
	}
...
func (r *GitRepositoryReconciler) SetupWithManager(mgr ctrl.Manager, notifyChan chan event.GenericEvent) error {
...
func reconcileLocalRepoContent(ctx context.Context, repo *v1alpha1.GitRepository, tgtRepo repoInfo, creds gitProviderCredentials, scheme *runtime.Scheme, tmplConfig v1alpha1.BuildCustomizationSpec, tmpDir string, repoMap *utils.RepoMap) error {
	...
	if push {
		remoteUrl, err := utils.FirstRemoteURL(tgtRepository)
		if err != nil {
...
func reconcileRemoteRepoContent(ctx context.Context, repo *v1alpha1.GitRepository, tgtRepo repoInfo, creds gitProviderCredentials, tmpDir string, repoMap *utils.RepoMap) error {
	logger := log.FromContext(ctx)
	srcRepo := repo.Spec.Source.RemoteRepository
	cloneDir := utils.RepoDir(srcRepo.Url, tmpDir)
	...
	remoteWT, _, err := utils.CloneRemoteRepoToDir(ctx, srcRepo, 1, false, cloneDir, "")
	if err != nil {
	...
	logger.V(1).Info("cloning repo", "repoUrl", tgtRepoSpec.Url, "fallbackUrl", getFallbackRepositoryURL(repo, tgtRepo), "cloneDir", tgtCloneDir)
	tgtRepoWT, tgtRepository, err := utils.CloneRemoteRepoToDir(ctx, tgtRepoSpec, 1, true, tgtCloneDir, getFallbackRepositoryURL(repo, tgtRepo))
	if err != nil {
		return fmt.Errorf("cloning repo %s: %w", srcRepo.Url, err)
	}

	err = utils.CopyTreeToTree(remoteWT, tgtRepoWT, fmt.Sprintf("/%s", repo.Spec.Source.Path), ".")
	if err != nil {
